...
class AccomplishmentReportGenerateXLSXService
{
    ...
    public function generate($events, $studentAccomplishments)
    {
        ...
        $table3 = $table3->map->only(['organization', 'title', 'budget', 'eventFundSource', 'eventNature', 'eventClassification', 'eventLevel', 'venue', 'start_date', 'end_date', 'start_time', 'end_time']);

        // X. OTHER STUDENT ACTIVITIES
        $table4 = $events->whereIn('event_category_id', [1,2,3,4]);
        $table4 = $table4->map->only(['organization', 'title', 'budget', 'eventFundSource', 'eventNature', 'eventClassification', 'eventLevel', 'venue', 'start_date', 'end_date', 'start_time', 'end_time']);
